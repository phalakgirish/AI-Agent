...
const MenuItemLink = ({ item, className }: SubMenus) => {
    return (
        <Link
            ...
            data-menu-key={item.key}
        >
            {item.icon && <i style={{ color: '#000' }} className={item.icon} />}
            {item.badge && (
                <span className={classNames('badge', 'bg-' + item.badge.variant, 'rounded-pill', 'float-end')}>
                    {item.badge.text}
                </span>
...
const AppMenu = ({ menuItems }: AppMenuProps) => {
    ...
    const userData:any = JSON.parse(StorageuserData) || null;
    const [role, setRole] = useState((userData != null)? userData.user_role_type.toString():'')
    ...
    return (
        <>
            <ul className="side-menu" ref={menuRef} id="side-menu" style={{ backgroundColor: '#e3e9ed' }}>
                {(menuItems || []).map((item, idx) => {
                    

                    return (
                        (item.usershow?.includes(role))?
